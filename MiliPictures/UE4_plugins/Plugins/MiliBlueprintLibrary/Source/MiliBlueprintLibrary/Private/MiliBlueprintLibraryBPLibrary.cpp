...
#include "MiliBlueprintLibraryPrivatePCH.h"
#include "MiliBlueprintLibraryBPLibrary.h"

#include "UnrealEd.h"
#include "ObjectTools.h"

UMiliBlueprintLibraryBPLibrary::UMiliBlueprintLibraryBPLibrary(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{

}

float UMiliBlueprintLibraryBPLibrary::MiliBlueprintLibrarySampleFunction(float Param)
{
	return -1;
}

bool UMiliBlueprintLibraryBPLibrary::ExportObjectToPath(class UObject* ObjectToExport, FString SaveFileName, FString FileNameAppendage, bool UseOriginalPath)
{
	...
	FString OriginalNamePath = SaveFileName;
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Export Object to Path unable to find exporter for %s!"), *ObjectToExport->GetName());
		return false;
	}
	...
	OriginalNamePath += FileNameAppendage + FString::Printf(TEXT(".%s"), *FirstExtension);
	UE_LOG(LogTemp, Log, TEXT("ExportObjectToPath Exporting \"%s\" to \"%s\""), *ObjectToExport->GetPathName(), *SaveFileName);

	// Create the path, then make sure the target file is not read-only.
	FString ObjectExportPath;
	if (UseOriginalPath == true)
	{
		ObjectExportPath = FString(FPaths::GetPath(OriginalNamePath));
	}
	else
	{
		ObjectExportPath = FString(FPaths::GetPath(SaveFileName));
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("ExportObjectToPath Failed to make directory %s"), *ObjectExportPath);
		return false;
	}
	else if (IFileManager::Get().IsReadOnly(*SaveFileName))
	{
		UE_LOG(LogTemp, Warning, TEXT("ExportObjectToPath Couldn't write to file '%s'. Maybe file is read-only?"), *ObjectExportPath);
		return false;
	}
	else
	{
		...
		if (ExporterToUse)
		{
			...
			if (UseOriginalPath == true)
			{
				Params.Filename = *OriginalNamePath;
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("ExportObjectToPath Failed to find exporter (last stage) for object: %s"), *ObjectToExport->GetName());
		}
	}


	UE_LOG(LogTemp, Warning, TEXT("ExportObjectToPath Failed for unknown reason for object: %s"), *ObjectToExport->GetName());
	return false;
