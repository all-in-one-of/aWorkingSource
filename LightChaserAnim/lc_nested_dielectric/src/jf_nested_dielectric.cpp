...
shader_evaluate
{
	...
	AtColor acc_refract_indirect_correct = AI_RGB_BLACK;
	if ( validInterface )
	{
...
//starting fixing bug///////////////////////////////////////////////////////////////
					...
					//AiMakeRay(&ray, AI_RAY_REFRACTED, &sg->P, &ray.dir, AI_BIG, sg);
						...
						if (refracted || do_disperse)
						{
							...
							else if ((AiV3Dot(ray.dir,sg->Nf) < 0.0f) && (ray.dir != AI_V3_ZERO))
							{
								...
								if (tracehit || refract_skies)
								{
									...
									//acc_refract_indirect += sample.color * weight * transmissionOnSample(&t2, &sample, true );
									//acc_refract_indirect += weight * transmissionOnSample(&t2, &sample, true );
								}
...
//ending fixing bug///////////////////////////////////////////////////////////////
				...
				if (trace_refract_direct && refracted )
				{
					...
					const bool use_refraction_btdf = false;
					float dr_roughnessU;
					float dr_roughnessV;
					int dr_btdf;
// those code would cause unpredictable noise
/*					if (use_refraction_btdf)
					...
					switch ( dr_btdf )
					{
						...
						case b_cook_torrance:
							// Cook Torrance
							AtVector U, V;
							if (!AiV3isZero(ppsg.dPdu) && AiV3Exists(ppsg.dPdu))
							{
							  // we have valid a valid dPdu derivative, construct V
							  AtVector Utmp = AiV3Normalize(ppsg.dPdu);
							  V = AiV3Normalize(AiV3Cross(ppsg.Nf, Utmp));
							  U = AiV3Normalize(AiV3Cross(V, ppsg.Nf));
							}
							else
							{
							  AiBuildLocalFramePolar(&U, &V, &ppsg.Nf);
							}

							//btdf_data_direct = AiCookTorranceMISCreateData(&ppsg, &U, &V, dr_roughnessU, dr_roughnessU);
							//btdf_data_direct2 = AiCookTorranceMISCreateData(&ppsg, &U, &V, drs_roughnessU, drs_roughnessU);
...
node_loader
{
	...
	node->name					  = "lc_nested_dielectric";
