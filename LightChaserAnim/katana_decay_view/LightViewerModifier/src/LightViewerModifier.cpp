                               ...
                               int numSegments, float roundness)
{
    ...
    double tan = tanf(a);
    double tmp = 1.0 / (1.0 + pow(tan, roundness));
...
public:
    ...
    void deepSetup(FnKat::ViewerModifierInput& input)
    {
        ...
        m_lightFilter = getArnoldLightFilter(input);
    ...
    void draw(FnKat::ViewerModifierInput& input)
    {
        ...
        //std::cout << "m_lightType:" << m_lightType << std::endl;
        //std::cout << "m_metaLightType:" << m_metaLightType << std::endl;
...
private:
    ...
    //=======================================================================
    std::vector<float> getArnoldLightDecayNear(FnKat::ViewerModifierInput& input)
    {
        std::vector<float> near_data;
        if(isUseNearAttenOn(input))
        {
            FnKat::FloatAttribute near_start = input.getGlobalAttribute("material.arnoldLightFilterParams.near_start");            
            FnKat::FloatAttribute near_end = input.getGlobalAttribute("material.arnoldLightFilterParams.near_end");
            near_data.push_back(near_start.getValue(0,false));
            near_data.push_back(near_end.getValue(0,false));
        }
        return near_data;   
    }

    std::vector<float> getArnoldLightDecayFar(FnKat::ViewerModifierInput& input)
    {
        std::vector<float> far_data;
        if(isUseFarAttenOn(input))
        {
            FnKat::FloatAttribute far_start = input.getGlobalAttribute("material.arnoldLightFilterParams.far_start");            
            FnKat::FloatAttribute far_end = input.getGlobalAttribute("material.arnoldLightFilterParams.far_end");
            far_data.push_back(far_start.getValue(0,false));
            far_data.push_back(far_end.getValue(0,false));
        }
        return far_data;   
    }
    std::vector<double> getArnoldLightScale(FnKat::ViewerModifierInput& input)
    {
        std::vector<double> result;
        FnKat::DoubleAttribute scaleAttr = input.getGlobalAttribute("xform.interactive.scale");            
        float tTime = scaleAttr.getSampleTime(0);
        FnKat::ConstVector<double> scaleVector = scaleAttr.getNearestSample(tTime);

        double scaleX = scaleVector[0];            
        double scaleY = scaleVector[1];
        double scaleZ = scaleVector[2];
    ...
    bool isUseNearAttenOn(FnKat::ViewerModifierInput& input)
    {
        FnKat::IntAttribute aMaterialAttr = input.getGlobalAttribute("material.arnoldLightFilterParams.use_near_atten");
        bool result = aMaterialAttr.getValue(0,false); 
        //std::cout << aMaterialAttr.getValue(0,false) << std::endl;
        return result;
    }
    
    bool isUseFarAttenOn(FnKat::ViewerModifierInput& input)
    {
        FnKat::IntAttribute aMaterialAttr = input.getGlobalAttribute("material.arnoldLightFilterParams.use_far_atten");        
        bool result = aMaterialAttr.getValue(0,false); 
        return result;
    }
    
    std::string getArnoldLightFilter(FnKat::ViewerModifierInput& input)
    {
        //std::cout << input.getFullName() << std::endl;
        FnKat::StringAttribute aMaterialAttr = input.getGlobalAttribute("material.arnoldLightFilterShader");
        if(!input.isSelected())
        {
            return "NoneType";   
        }
        if(aMaterialAttr.isValid())
        {
            std::string lightFilter = aMaterialAttr.getValue("", false);
            return lightFilter;
        }
        else
        {
    ...
    void drawArnoldLightFilter(FnKat::ViewerModifierInput& input,std::string light = "light")
    {
        glPushMatrix();

        std::vector<double> scale = getArnoldLightScale(input);
        double x = scale[0];
        double y = scale[1];
        double z = scale[2];
        ...
        if(light == "point_light")
        {
            ...
            if(isUseNearAttenOn(input))
            {
                std::vector<float> near = getArnoldLightDecayNear(input);
                float near_start = near[0];
                float near_end = near[1];

                glPushMatrix();
                glColor3f(0,1,0);
                
                glPushMatrix();
                drawCircle(0,0,near_start,120);
                drawCircle(0,0,near_end,120);
                glPopMatrix();
            ...
            if(isUseFarAttenOn(input))
            {
                std::vector<float> far = getArnoldLightDecayFar(input);
                float far_start = far[0];
                float far_end = far[1];
                ...
                glPushMatrix();
                drawCircle(0,0,far_start,120);
                drawCircle(0,0,far_end,120);
                glPopMatrix();
        ...
        else
        {
            ...
            {
                std::vector<float> near = getArnoldLightDecayNear(input);
                float near_start = near[0]*dire;
                float near_end = near[1]*dire;
                glBegin(GL_LINE_LOOP);
                glColor3f(0,1,0);
                glVertex3f(1.5,-1.5,near_start);
                glVertex3f(1.5,1.5,near_start);
                glVertex3f(-1.5,1.5,near_start);
                glVertex3f(-1.5,-1.5,near_start);
                glEnd();
            ...
            {
                std::vector<float> far = getArnoldLightDecayFar(input);
                float far_start = far[0]*dire;
                float far_end = far[1]*dire;
                glBegin(GL_LINE_LOOP);
                glColor3f(1,0,0);
                glVertex3f(1.5,-1.5,far_start);
                glVertex3f(1.5,1.5,far_start);
                glVertex3f(-1.5,1.5,far_start);
                glVertex3f(-1.5,-1.5,far_start);
                glEnd();
            ...
            glVertex3f(0.0f,0.0f,length*dire);
            glEnd();
    ...
    void drawDiskLight(FnKat::ViewerModifierInput& input)
    {
        ...
        //==============================
        // draw Arnold light decay shape
        if(m_lightFilter == "light_decay")
        {
            drawArnoldLightFilter(input);            
        }
    ...
    void drawCylinderLight(FnKat::ViewerModifierInput& input)
    {
        ...
        if(m_lightFilter == "light_decay")
        {
            drawArnoldLightFilter(input,"cylinder_light");            
        }
    }
    ...
    void drawPointLight(FnKat::ViewerModifierInput& input)
    {
        ...
        if(m_lightFilter == "light_decay")
        {
            drawArnoldLightFilter(input,"point_light");            
        }
    ...
    void drawSpotLight(FnKat::ViewerModifierInput& input)
    {
        ...
        if(m_lightFilter == "light_decay")
        {
            drawArnoldLightFilter(input,"spot_light");            
        }
...
void registerPlugins()
{
    ...
    std::cout << "Register LightViewerModifier v1.0" << std::endl;
}
