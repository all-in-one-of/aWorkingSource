...
args = argsChaser()
__FilePath__ = args["file"]
__LogPath__ = args["logs"]
__Mode__ = args["mode"]
...
def recoverScript(katanaFile, jsonFile, logsFile):
    from Katana import KatanaFile,NodegraphAPI
    # if katanaFile is not None:
    KatanaFile.Load(katanaFile)
...
def main():
    json_path = os.path.join(getMaterialXJsonDir(__FilePath__), 'translate.json')
    if __Mode__ == "write":
        writeScript(__FilePath__, json_path)
    elif __Mode__ == "recover":
        recoverScript(__FilePath__, json_path, __LogPath__)
    else:
        check(__FilePath__)
