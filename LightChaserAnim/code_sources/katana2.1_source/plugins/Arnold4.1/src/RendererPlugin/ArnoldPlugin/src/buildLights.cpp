...
AtNode* buildLight(FnScenegraphIterator lightIterator, std::vector<AtNode*>* childNodes, ArnoldPluginState* sharedState)
{
    ...
    else if (pystring::find(lightType, "quad_light") > -1)
    {
      ...
      if (AiNodeEntryLookUpParameter(baseNodeEntry, "vertices"))
      {
            ...
            verts.reserve(4*3*sampleTimes.size());
            
            for (std::vector<float>::iterator I = sampleTimes.begin();
                    I!=sampleTimes.end(); ++I)
            {
                float sampleTime = (*I);
                
                std::map<float, Imath::M44d>::iterator J = xformSamples.find(sampleTime);
                
                if (J == xformSamples.end())
                {
                    AiMsgWarning(
                    std::string("[kat] cannot find matching time samples for transformation of " +
                        lightIterator.getFullName()).c_str());
                    return lightNode;
                }
                
                Imath::M44d & xform = (*J).second;
            ...
            AiNodeSetArray(lightNode, "vertices", AiArrayConvert(4, sampleTimes.size(), 
                    AI_TYPE_POINT, &verts[0]));
            
            if (sampleTimes.size() > 1)
            {
                if (AiNodeEntryLookUpParameter(baseNodeEntry,
                        "time_samples"))
                {
                    AiNodeSetArray(lightNode, "time_samples",
                                AiArrayConvert(sampleTimes.size(), 1, AI_TYPE_FLOAT,
                                        &sampleTimes[0]));
                }
