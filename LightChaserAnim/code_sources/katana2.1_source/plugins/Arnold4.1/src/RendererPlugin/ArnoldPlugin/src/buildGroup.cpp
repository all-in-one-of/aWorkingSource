...
AtNode* buildGroup(FnScenegraphIterator sgIterator, std::vector<AtNode*>* childNodes, bool forceExpand, bool nooverride, ArnoldPluginState* sharedState, bool obeyInstanceId)
{
    ...
    /*
    This walking represents the main traversal of the scenegraph, and as such
    is where we must explicit choose to walk the scenegraph, while evicting
    un-used scene data.
    
    There are many plausible strategies for eviction, but the simplest
    and most obvious are to evict for each 'main-line' traversal, or,
    to evict on each getFirstChild call.
    
    In the interest of performance (and potential re-use of location data)
    we have chosen for the moment to only EVICT_ON_CHILD_TRAVERSAL, but future
    ...
    (other than as an upper limit memory bound) as this would make scenegraph
    ...
    */
    ...
    FnScenegraphIterator child = sgIterator.getFirstChild(EVICT_ON_CHILD_TRAVERSAL);
    if (!child.isValid()) return NULL;
    ...
    if (forceExpand || !worldBoundAttr.isValid())
    {
        while (child.isValid())
        {
            buildLocation(child, childNodes, forceExpand, nooverride, sharedState, obeyInstanceId);
            child = child.getNextSibling(EVICT_ON_SIBLING_TRAVERSAL);
        }
        return 0;
    }
    ...
    AiNodeSetPtr(procedural, "userptr", new FnScenegraphIterator(child));
