...
node_parameters
{
   ...
   AiParameterVec("normal_map", 0.f,0.f,0.f);
   AiParameterStr("fuse_set", "");   
}
...
node_update
{
   ...
   data->hasChainedNormal = AiNodeIsLinked(node, "normal_map");
   data->fuse_set = AiNodeGetStr(node,"fuse_set");
...
shader_evaluate
{
   ...
   else 
   {
      ...
      while(AiSamplerGetSample(samit, samples)) 
      {
         ...
         {
            AtString fuse_tag;
            bool isFuseSet = AiUserGetStrFunc(AtString("fuse_tag"),&hitpoint,fuse_tag);
            if(!data->fuse_set.empty())
            {
               if(isFuseSet && fuse_tag == data->fuse_set)
                  /*just go on*/;
               else
                  continue;
            }
   ...
   if(data->hasChainedNormal) // if lc_fuse_normal normal_map parameter has be connected with normal map
   {
      ...
      {
         AtVector normal_map = AiShaderEvalParamVec(p_normal_map);
         //AtVector result = AiV3Normalize(sg->Nf + normal_map);
         sg->out.VEC() = normal_map;         
