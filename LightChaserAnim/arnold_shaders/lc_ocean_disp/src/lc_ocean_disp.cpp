...
#ifdef WRITE_OPENEXR
#include "common/openEXROutput.h"
#endif /* WRITE_EXR */
...
node_update
{
    ...
    int resolution = AiNodeGetInt(node,AtString("resolution"));
    unsigned int  spectrum;
    unsigned int seed = AiNodeGetInt(node,AtString("seed"));
    float   oceanScale = AiNodeGetFlt(node,AtString("oceanScale"));
    float   oceanDepth = AiNodeGetFlt(node,AtString("oceanDepth"));
    float   surfaceTension = AiNodeGetFlt(node,AtString("surfaceTension"));
    float   velocity = AiNodeGetFlt(node,AtString("velocity"));
    float   cutoff = AiNodeGetFlt(node,AtString("cutoff"));
    float   windDir = AiNodeGetFlt(node,AtString("windDir"));
    int     windAlign;
    float   damp;
    float   waveSpeed;
    float   waveHeight;
    ...
    // main input function
    pOcean->input(
        AiNodeGetInt(node,AtString("resolution")),
        AiNodeGetInt(node,AtString("seed")),
        AiNodeGetFlt(node,AtString("oceanScale")),
        AiNodeGetFlt(node,AtString("oceanDepth")),
        AiNodeGetFlt(node,AtString("surfaceTension")),

        AiNodeGetFlt(node,AtString("velocity")),
        AiNodeGetFlt(node,AtString("cutoff")),
        AiNodeGetFlt(node,AtString("windDir")),

        AiNodeGetInt(node,AtString("windAlign")),

        AiNodeGetFlt(node,AtString("damp")),
        AiNodeGetFlt(node,AtString("waveSpeed")),
        AiNodeGetFlt(node,AtString("waveHeight")),
        AiNodeGetFlt(node,AtString("chopAmount")),
        AiNodeGetFlt(node,AtString("time")),
        AiNodeGetFlt(node,AtString("repeatTime")),
    ...
    if(pOcean->isChoppy() && !rawOutput)
    {
        ...
        //    AiMsgWarning("[aaOcean Shader] Foam Min/Max mismatch. Please set the Foam Min/Max values in foam shader to Min: %f, Max: %f", outMin, outMax);
    }
}
    ...
    if(pOcean->isChoppy())
    {
        ...
        if(!AiShaderEvalParamBool(p_raw))
        {
            ...
            if(AiShaderEvalParamBool(p_invertFoam))
                sg->out.RGBA.a  = 1.0f - CLAMP(sg->out.RGBA.a, fmin, fmax);
            else
                sg->out.RGBA.a  = CLAMP(sg->out.RGBA.a, fmin, fmax);

            // removing negative leftovers
            sg->out.RGBA.a  = std::max(sg->out.RGBA.a, 0.0f);
            // apply gamma
