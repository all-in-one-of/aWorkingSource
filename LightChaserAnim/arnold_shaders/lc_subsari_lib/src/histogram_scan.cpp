...
shader_evaluate
{
  ...
  AtVector levelled = {0.0f,0.0f,0.0f};
  for (int i = 0; i != 3; ++i)
  {
  float val = outBlack + (input[i] - InBlack) * grad;
  levelled[i] = pow( max(val, 0.0f), alpha);
  }
  AtVector MAX_V = {1.0f,1.0f,1.0f};
  AtVector MIN_V = {0.0f,0.0f,0.0f};

  if(clamp_in)
  {
  // Clamp to 0-1 range
  levelled = kt::clamp( levelled, MIN_V, MAX_V);
  }
  AtVector result = kt::clamp(MAX_V - levelled, MIN_V, MAX_V);
  sg->out.RGB = kt::AtVector2AtRGB(result);
