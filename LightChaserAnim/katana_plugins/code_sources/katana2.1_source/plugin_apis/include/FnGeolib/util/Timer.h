...
FNGEOLIBUTIL_NAMESPACE_ENTER
{
    ...
    public:
        ...
        const Counter& operator += (double amt) {_accum += amt; ++_count; return *this;}

        void setOutputStream(std::ostream &ostr) {_ostr = &ostr;}
    ...
    {
    public:
        Timer(const std::string& name="Unnamed", Counter *accum = NULL, bool print = true) : _ostr(&std::cerr)
        {
            _name = name;
            _accum = accum;
            _print = print;
    ...
    class MillisecondFormator
    {
        ...
        public :
            MillisecondFormator(float timeInMillisconds) : _timeInMillisconds(timeInMillisconds){}
            friend std::ostream& operator <<(std::ostream &os,const MillisecondFormator &formator)
            {
