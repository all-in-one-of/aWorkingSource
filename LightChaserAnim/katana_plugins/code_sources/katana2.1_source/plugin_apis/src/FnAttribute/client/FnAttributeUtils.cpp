        ...
        switch (type)
        {
            ...
            case kFnKatAttributeTypeFloat:
            {
                ...
                for (int64_t t = 0; t < ref_times; ++t)
                {
                    ...
                    if (data.size() > 0)
                    {
                        floatval.fillInterpSample(&data[0], data.size(),
            ...
            case kFnKatAttributeTypeDouble:
            {
                ...
                for (int64_t t = 0; t < ref_times; ++t)
                {
                    ...
                    if (data.size() > 0)
                    {
                        doubleval.fillInterpSample(&data[0], data.size(),
                            timeAttr.getSampleTime(t));
                    }
                }
                return builder.build();
    ...
    namespace
    {
        ...
        Attribute _RemoveTimeSamplesIfAllSame(const T & attr)
        {
            ...
            for (int64_t i = 1; all_same && (i < nt); i++)
            {
                ...
                all_same &= (memcmp(&v0[0], &v1[0], v0.size() * sizeof(typename T::value_type)) == 0);
            }

            if (all_same)
            {
                return T(&v0[0], attr.getNumberOfValues(), attr.getTupleSize());
            }
            else
            {
                return attr;
