...
namespace
{
    // handle to the GEOLIB dso
    static void* gGeolibSO = 0x0;
    static FnPluginHost* gHost = 0x0;
}

RenderOutputUtilsSuite_v3* _suite = 0x0;

bool bootstrapGEOLIB(const std::string& katanaPath)
{
    // The GEOLIB library will be in $KATANA_ROOT/bin
#ifdef _WIN32
#ifdef _DEBUG
    std::string geolibPath = katanaPath + "/bin/GeoAPI_cmodule_d.pyd";
#else
    ...
    //load GEOLIB
    gGeolibSO = dlopen(geolibPath.c_str(), RTLD_NOW);
    if(!gGeolibSO)
    {
        //TODO: log
        std::cout << "Error loading GEOLIB: " << dlerror() << " Loading Path: " << geolibPath.c_str() << std::endl;
        return false;
    }

    // Get the getGeolibHost() function from GEOLIB
    void* func = dlsym(gGeolibSO, "getGeolibHost");
    if(!func)
    {
        //FnLogWarn(dlerror());
    ...
    setHost(host); // on RenderOutputUtils
        ...
        for(int64_t i = 0; i < numSampleTimes; ++i)
        {
            ...
            else if(name.compare(0, 6, "matrix") == 0)
            {
                if(numberOfSamples == 16)
                {
                    ...
                    if(invertMatrix)
                        m.invert();
                    std::vector<float> values(m.getValues(), m.getValues()+16);
                                           ...
                                           float shutterClose)
{
    ...
    FnAttribute::FloatAttribute result =
        FnAttribute::Attribute::CreateAndSteal(
            _suite->findSampleTimesRelevantToShutterRange(&inputSamplesVector[0],
