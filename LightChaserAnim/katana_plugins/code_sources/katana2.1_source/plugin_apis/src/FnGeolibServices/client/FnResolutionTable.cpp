...
FNGEOLIBSERVICES_NAMESPACE_ENTER
{
    ...
    namespace // anonymous
    {
        ...
        {
            static const FnResolutionTableHostSuite_v1 * suite = 0x0;

            if (!suite)
            {
                suite = reinterpret_cast<const FnResolutionTableHostSuite_v1 *>(
                        FnPluginManager::PluginManager::getHostSuite(
                                "ResolutionTableHost", 1));
            }
    ...
    namespace FnResolutionTable
    {
        bool hasResolution(const std::string & name)
        {
            ...
            return static_cast<bool>(suite->hasResolution(name.c_str()));
        }
