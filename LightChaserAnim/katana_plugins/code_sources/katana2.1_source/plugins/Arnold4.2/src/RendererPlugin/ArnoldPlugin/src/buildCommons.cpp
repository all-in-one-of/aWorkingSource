...
void setArnoldParam_String(GroupAttribute globals, const char* attrName, AtNode* node, const char* arnoldName)
{
    ...
    if (!attrRef.isValid())
    {
        ...
        AiNodeResetParameter(node, arnoldName);
    }
    else
    {
        if (AiParamGetType(pentry) == AI_TYPE_ARRAY)
        ...
        else if (AiParamGetType(pentry) == AI_TYPE_ENUM)
        {
            AtEnum arnoldEnum = AiParamGetEnum(pentry);
            if (arnoldEnum)
            {
                int enumValue = AiEnumGetValue(arnoldEnum, attrRef.getValue().c_str());
                if (enumValue != -1)
                {
                    AiNodeSetInt(node, arnoldName, enumValue);
                }
                else
                {
                    AiMsgWarning("[kat] attempt to set \"%s\" on node \"%s\" failed "
                        "because of invalid enum string.",
                                arnoldName, AiNodeGetName(node));
                    AiNodeResetParameter(node, arnoldName);
                }
            }
            else
            {
                AiMsgWarning("[kat] attempt to set \"%s\" on node \"%s\" failed "
                    "because Arnold enum was inaccessible.",
                            arnoldName, AiNodeGetName(node));
                AiNodeResetParameter(node, arnoldName);
            }
        }
...
// TODO: this function is totally useless; there is no 'tranform' node in SA Arnold
...
{
    AtByte visibilityMask = AI_RAY_ALL & (~AI_RAY_CAMERA);
    ...
    if (visAttr.isValid())
    {
        ...
        rayAttr = visAttr.getChildByName("AI_RAY_SUBSURFACE");
        if (rayAttr.isValid() && !rayAttr.getValue())
        {
