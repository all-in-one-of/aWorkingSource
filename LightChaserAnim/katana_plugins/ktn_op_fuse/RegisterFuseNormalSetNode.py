...
def registerFuseNormalSet():
    ...
    def buildFuseNormalSetOpChain(node, interface):
        """
        ...
        tagParam = node.getParameter('fuse_set')
        if tagParam:
            argsGb.set('fuse_set', tagParam.getValue(frameTime))

        # Add the Messer Op to the Ops chain
        interface.appendOp('FuseNormalSet', argsGb.build())


    # Create a NodeTypeBuilder to register the new type
    nodeTypeBuilder = Nodes3DAPI.NodeTypeBuilder('FuseNormalSet')
    ...
    gb.set('fuse_set', FnAttribute.StringAttribute(''))
    ...
    nodeTypeBuilder.setBuildOpChainFnc(buildFuseNormalSetOpChain)

    # Build the new node type
    nodeTypeBuilder.build()

# Register the node
registerFuseNormalSet()
