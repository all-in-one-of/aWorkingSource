...
The asset ID is defined in the following way:
...
or, if this is not set, default to tempfile.gettempdir().
Asset files will be saved in a directory structure like:
[base_dir]/[show]/[shot]/[version]
...
be stored. For example, for an asset of the type 'image' with attributes
...
log = logging.getLogger("PyMockAsset")
...
class MockAsset (AssetAPI.BaseAssetPlugin):
    """
    The main class of the plug-in that will be registered as "PyMockAsset". It
    ...
    def containsAssetId(self, string):
        ...
        """
        return "mock://" in string
    
    
    @LogCall
    def resolveAsset(self, assetId):
        """
        ...
        if not self.isAssetId(assetId):
            ...
            log.warning("Asset ID '%s' is not a valid for PyMockAsset. "
