...
# the size of LightNameSets should be same as the size of ShortcutSets!
def ShortcutGenarater(PackageActionHash):
    light_name = PackageActionHash[15:]
    SortedLightNameSets = sorted(LightNameSets)
    index = SortedLightNameSets.index(light_name)
    return ShortcutSets[index]
...
class ArnoldSpotLightGafferUIDelegate(ArnoldLightGafferUIDelegate):
    ...
    DefaultShortcut = ShortcutGenarater(AddPackageActionHash)
...
class ArnoldFilterLightGafferUIDelegate(ArnoldLightGafferUIDelegate):
    AddPackageActionHash = 'GafferThree-AddFilterLight'
    DefaultShortcut = ShortcutGenarater(AddPackageActionHash)
    def getMaterialTabPolicy(self):
        # Create a new material policy that just has the prmanLightParams for
        # each light type
