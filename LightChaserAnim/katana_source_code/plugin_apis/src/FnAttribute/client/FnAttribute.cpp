...
FNATTRIBUTE_NAMESPACE_ENTER\
{
    ...
    {
        // Open the FnAttributeHost library and get its
        std::string attrHostLibPath = katanaPath + "/bin/libFnAttributeHost.so";
        void *dsoptr = dlopen(attrHostLibPath.c_str(), RTLD_NOW);
        if (!dsoptr)
        {
            std::cerr << "Error loading FnAttributeHost: " << dlerror() << std::endl;
            return false;
        }
        ...
        if (!funcptr)
        {
            std::cerr << "Error loading ATTRUTIL: " << dlerror() << std::endl;
            dlclose(dsoptr);
            return false;
        }
...
namespace
{
    ...
    std::string Attribute::getXML() const
    {
        ...
        if(buf.size()>0) return std::string(&buf[0], buf.size());
        return "";
    }
...
namespace
{
                                         ...
                                         FnKatStreamType streamType) const
    {
        ...
        bool returnval = static_cast<bool>(getSuite()->writeAttributeStream(
            _handle, stream, func, &errorMessageHandle, streamType));
        ...
        int64_t tupleSize) : DataAttribute()
    {
        ...
        if(!cstrvec.empty()) cstrvals = const_cast<const char **>(&cstrvec[0]);
        ...
        int64_t tupleSize) : DataAttribute()
    {
        ...
        if(!stringvec.empty()) values = const_cast<const char **>(&cstrvec[0]);
        ...
        const bool inheritChildren) : Attribute(0x0)
    {
        ...
        FnAttributeHandle handle = getSuite()->createGroupAttr(&names[0],
            &handles[0], (int64_t)names.size(), (uint8_t) inheritChildren);
        stealHandle(handle);
    }
