...
{
    // TODO: allow these to be reset once live rendering gets the new values appropriately
...
AtNode* buildCameraLocation(FnScenegraphIterator cameraIterator, ArnoldPluginState* sharedState, int *overscan, int *displayWindowSize)
{
    ...
    //shutter
    {
        ...
        FnAttribute::FloatAttribute rollingShutterDurationAttr = rootIterator.getAttribute("arnoldGlobalStatements.rolling_shutter_duration");
        if (rollingShutterAttr.isValid())
        {
            AiNodeSetFlt(cameraNode, "rolling_shutter_duration", rollingShutterDurationAttr.getValue());
        }
    }
